...
public class TreeImpl<T> implements Tree<T> {
	...
	@Override
	public void add(Tree<T> oTreeToAdd, T oParentValue) {
 		if (oTreeToAdd == null || oParentValue == null) {
			
			throw new RuntimeException("Invalid value or parent");
		}
		
		if (searchValue(oTreeToAdd.getValue()) != null) {
		...
		((TreeImpl<T>) oParent).childs.add(oTreeToAdd);
